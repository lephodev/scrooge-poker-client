...
const Results = ({ tournamentData }) => {
  ...
  useEffect(() => {
    ...
    tournamentData?.eleminatedPlayers.reverse().forEach(el=>{
        const winnrPlayr = allWinnerPlayers.find(winnr => winnr.userId === el.userid );
        if(winnrPlayr){
          players.push({
            name: el.name,
              ...
              <>
                {
                  allPlayers?.map((el, i)=>{
                    if(i === 0){
                      ...
                      return (<tr className="secondRank myRank">
                      <td>
                        <p>2</p>
                        <Star />
                      </td>
